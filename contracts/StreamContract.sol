...
contract StreamContract is AccessControlUpgradeable, IERC777Recipient {
    ...
    constructor(address _ibAlluoFrom, address _ibAlluoTo) {
        ...
        idaV1Lib = IDAv1Library.InitData(
            host,
    ...
    function getIbAlluoFrom(uint256 amount, address token)
        public
    ...
    function startAluoStream(int96 flowRatePerSecond)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        IIbAlluo(ibAlluoFrom).createFlow(
            address(this),
            flowRatePerSecond,
            IIbAlluo(ibAlluoFrom).balanceOf(address(this))
        );
    }

    function endAluoStream() public onlyRole(DEFAULT_ADMIN_ROLE) {
        IIbAlluo(ibAlluoFrom).deleteFlow(address(this));
    }
    ...
    function checkBalances()
        ...
        view
        returns (
            int256 ibAlluoFromRealTimeBalance,
            int256 ibAlluoToRealtimeBalance
        )
    {
        ibAlluoFromRealTimeBalance = IIbAlluo(ibAlluoFrom).getBalance(
            address(this)
        );
        ibAlluoToRealtimeBalance = IIbAlluo(ibAlluoTo).getBalance(
            address(this)
        );
    }
    ...
    function transferAllFunds() public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        IIbAlluo(ibAlluoFrom).transfer(
            msg.sender,
            uint256(ibAlluoFromRealTimeBalance)
        );
        IIbAlluo(ibAlluoTo).transfer(
            msg.sender,
            uint256(ibAlluoToRealtimeBalance)
        );
    }
